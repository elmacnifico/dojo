...
* **Startup Plan:** A `startup.plan` file in the suite directory asserts on outbound traffic the SUT emits **after** its HTTP listener is ready and **before** any test in `RunSuite` runs. It may contain **only** `Expect` lines (no `Perform`). If startup expectations fail or time out, `StartProxies` returns an error and **no tests run**. See the `readme.md` and the example suite’s [example/tests/blackbox/startup.plan](example/tests/blackbox/startup.plan).
* **Configuration:** Technical details (URLs, timeouts) live in `dojo.yaml`. The DSL (`test.plan`) remains purely logical.
* **Matching:** Outbound traffic is tied to the active test by **normalized matching** between the resolved `expected_request` fixture and the actual payload (SQL: collapse whitespace outside string literals and strip a trailing `;`, containment match; HTTP: canonical-JSON subset match). There is no separate correlation block for routing. **Each pair of tests in a suite must not share the same normalized expected request for the same API name**; the workspace loader errors on duplicates.
...
## Documentation Rules (README vs SKILL)
* **`readme.md`**: Must contain the FULL documentation for the project. Do NOT truncate or move the bulk of the documentation out of the README. It should include the introduction, architecture, how to write suites, the DSL reference, fixtures, concurrency best practices, and CLI usage.
* **`docs/dojo-skill.md` (Cursor Skill)**: This file should be a focused, concise guide specifically tailored for Cursor agents writing `.plan` files. It should NOT replace the `readme.md`. When updating documentation, ALWAYS update the `readme.md` first, and only copy relevant suite-authoring instructions to the skill file. NEVER delete content from the `readme.md` just because it exists in the skill file.
